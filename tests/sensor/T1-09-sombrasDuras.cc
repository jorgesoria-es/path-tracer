...
int main() {
    ...
    std::ofstream log("T1-09-sombrasDuras.log");
    log << "[T1-09] Análisis sombras duras iniciado\n";
    ...
    escena.add(std::make_shared<LuzPuntual>(
        Punto(Eigen::Vector3d(0.0, 0.9, 0.0)),   
        Pixel(0.5, 0.5, 0.5)                 
    ));
    ...
    for (int spp : spps) {
        ...
        renderer.render(hdr, ldr, RenderMode::DIRECT);
