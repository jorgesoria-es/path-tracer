...
int main(int argc, char** argv) {
    const std::string nomTest = "test_t1-03-ref";
    ...
    // Esfera izquierda (refractante)
    escena.add(std::make_shared<Esfera>(
        Punto(Eigen::Vector3d(-0.5, -0.7, 0.25)),           // centro
        0.3,                                                // radio
        Pixel(), Pixel(), Pixel(), Pixel(1.0, 1.0, 1.0),    // emisión, difuso, especular, transmitancia
        1.6                                                 // índice refracción
    ));
    ...
    escena.add(std::make_shared<Esfera>(
        Punto(Eigen::Vector3d(0.5, -0.7, -0.25)),           // centro
        0.3,                                                // radio
        Pixel(), Pixel(0.5, 0.9, 0.9), Pixel(), Pixel()     // emisión, difuso, especular, transmitancia
    ));

    //----------Luces
    escena.add(std::make_shared<LuzPuntual>(
        Punto(Eigen::Vector3d(0.0, 0.5, 0.0)),              // centro
        Pixel(1.0, 1.0, 1.0)                                // potencia
    ));
