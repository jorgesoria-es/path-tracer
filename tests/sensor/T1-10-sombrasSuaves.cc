...
int main() {
    ...
    std::ofstream log("T1-10-sombrasSuaves.log");
    log << "[T1-10] Análisis de sombras suaves iniciado\n";
    ...
    escena.add(std::make_shared<Esfera>(
        Punto(Eigen::Vector3d(-0.5, -0.7, 0.25)),   // más cerca de la pared roja
        0.3,
        Pixel(),                    // emisión
    ...
    escena.add(std::make_shared<Esfera>(
        Punto(Eigen::Vector3d(0.5, -0.7, -0.25)),   // más cerca de la pared verde
        0.3,
        Pixel(),                    // emisión
        Pixel(0.9, 0.9, 0.9),       // difuso blanco
        Pixel(),                    // especular
